...
vignette: >
  %\VignetteIndexEntry{Getting started with maestro}
...
`maestro` enchaine telechargement IGN, extraction de patches multi-modaux,
et inference des modeles MAESTRO (classification par patch, 13 essences
PureForest) et FLAIR (segmentation pixel, 15 classes CoSIA), publies sur
Hugging Face par l'IGN.
...
```{r}
maestro::configurer_python()
```

`configurer_python()` cree un environnement conda dedie (`maestro-py`) si
necessaire et installe les dependances : `torch`, `transformers`,
...
2026). Sous le seuil de couverture, `prepare_dem()` rebascule sur DSM =
DTM avec un warning ; passez `allow_dtm_only_fallback = FALSE` pour
retirer la modalite a la place.
...
`inst/scripts/cloud_train_pureforest.sh` et la vignette
[Pipeline Python](python-pipeline.html).
...
a 0,2 m. Voir `flair_models()` pour la liste des modeles supportes.

## Et apres

- Le tour d'horizon de la stack Python (entrainement, fine-tuning) :
  vignette [Pipeline Python](python-pipeline.html).
- Toutes les fonctions par theme : [Reference](../reference/index.html).
